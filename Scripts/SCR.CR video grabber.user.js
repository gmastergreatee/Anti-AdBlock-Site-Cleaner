...
// @version      0.1.2
// @description  This userscript works only if the current page's video contains an ad(embedded in the player), otherwise keep watching from the site
...
$(document).ready(function () {
    ...
    function getLink(index) {
        if (index < videoDataList.length) {
            $.get(atob('aHR0cHM6Ly9zY3IuY3IvQUxQSEEtc291cmNlLVRWLnBocD9laWQ9') + $(videoDataList[index]).data('eid') + '&t=' + t_value)
                ...
                .fail(function () {
                    $.get(atob('aHR0cHM6Ly9zY3IuY3IvUkFQSUQtc291cmNlLVRWLnBocD9laWQ9') + $(videoDataList[index]).data('eid') + '&t=' + t_value)
                        .done(function (response) {
    ...
    function checkFailed() {
        if (main_playlist1080.length == 0 && main_playlist360.length == 0 && main_playlist480.length == 0 && main_playlist720.length == 0) {
            alert('Sorry, no videos with ads found');
        }
    }
